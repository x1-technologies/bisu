...
trim_v3() {
    ...
    # choose fast bash path or awk path based on length
    len=${#str}
    if ((len >= critical_point)); then
        use_awk=1
    else
    ...
    if [[ "$chars" =~ ^[[:space:]]*$ ]]; then
        if ((use_awk == 0)); then
            # small input: pure-Bash trimming (fast)
            case $endpoints in
            1) # left only
                str="${str#"${str%%[![:space:]]*}"}"
                ;;
            ...
            if [[ "$endpoints" == "1" ]]; then
                str=$(
                    awk -v chars="[[:space:]]" -v IGNORECASE="$ci" '
                    { gsub("^" chars "+", ""); print }
                    ' <<<"$str" 2>/dev/null
                )
                ...
                # exact archived-style awk for both ends
                str=$(
                    awk -v chars="[[:space:]]" -v IGNORECASE="$ci" '
                    {
                        gsub("^" chars "+", "")
    ...
    # ---- custom chars branch ----
    if ((use_awk == 1)); then
        # large input: fallback to awk (preserve archived logic for custom classes)
        if [[ "$endpoints" == "1" ]]; then
            str=$(
