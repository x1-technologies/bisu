...
command_exists_async() {
    ...
    local cmds=("$@")  # Array of commands from function arguments
    ...
    for i in "${!cmds[@]}"; do
        (
            local c
            c=$(echo "${cmds[i]}" | xargs) # Trim whitespace (assumed xargs exists)
        ...
        pids[i]=$! # Capture background job PID
    done
