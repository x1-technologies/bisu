...
# Define BISU VERSION
export BISU_VERSION="10.2.530"
# Set BISU's last release date
export BISU_LAST_RELEASE_DATE="2025-08-27Z"
# Minimal Bash Version
export MINIMAL_BASH_VERSION="5.0.0"
# BISU info uri
export BISU_INFO_URI="https://bisu.dev"
# Default title
...
# Set a critical point for adopting awk to trim
export TRIM_CRITICAL_POINT=102400
# Error message prefix
export ERROR_MSG_PREFIX="[<fg_orange>Error</fg_orange>] "
# BISU File URL
export BISU_FILE_URL="https://g.bisu.dev/bisu-file"
# BISU ASC File URL
export BISU_ASC_FILE_URL="https://g.bisu.dev/bisu-asc-file"
# Global Variables
...
export TARGET_PATH_PREFIX="/usr/local/bin"                             # Default target path for moving scripts
...
declare -a CURRENT_ARGS=("$@") && export CURRENT_ARGS
...
UTILITY_VERSION=${UTILITY_VERSION:-"1.0"}
# Set this utility's last release date
LAST_RELEASE_DATE=${LAST_RELEASE_DATE:-}
# Set this utility's doc URI
UTILITY_INFO_URI=${UTILITY_INFO_URI:-}
# Set this utility's asc sig file URL
UTILITY_ASC_FILE_URL=${UTILITY_ASC_FILE_URL:-}
...
is_valid_var_name() {
    local var_name="$1"
    var_name="${var_name#"${var_name%%[![:space:]]*}"}" # ltrim
    var_name="${var_name%"${var_name##*[![:space:]]}"}" # rtrim
...
# BISU file name
bisu_filename() {
    printf '%s' $(basename $(bisu_file_path))
}
...
output() {
    ...
    [[ $message == *\<*\>* ]] && {
        ...
        if is_func "$FUNC_STYLE_CODE_INJECTION"; then
            ...
            [ -n "$injected_style_codes" ] && {
                ...
                assoc_array_merge "style_codes" "injected_style_codes" "style_codes"
            }
        fi
    ...
    if [[ "$log_only" == "true" ]]; then
        eval "$(printf '%s ' "$command")" |
            ...
            tee -a -- "$(current_log_file)" &>/dev/null ||
            return 1
    else
        eval "$(printf '%s ' "$command")" |
            { debug_mode_on && fold -s -w "$LINE_BREAK_LENGTH" || fold -s -w "$LINE_BREAK_LENGTH" 2>/dev/null; } |
            { debug_mode_on && tee -a -- "$(current_log_file)" || tee -a -- "$(current_log_file)" 2>/dev/null; } ||
            return 1
    fi
...
command_exists() {
    ...
    local strict_mode=$(trim "${2:-false}")
...
process_is_running() {
    local pid="$1"
    pid="${pid#"${pid%%[![:space:]]*}"}" # ltrim
    pid="${pid%"${pid##*[![:space:]]}"}" # rtrim
...
quit() {
    ...
    pid="${pid%"${pid##*[![:space:]]}"}" # rtrim
    local quit_group="$3"
    quit_group="${quit_group#"${quit_group%%[![:space:]]*}"}" # ltrim
    quit_group="${quit_group%"${quit_group##*[![:space:]]}"}" # rtrim
    local force="$4"
    force="${force#"${force%%[![:space:]]*}"}" # ltrim
    force="${force%"${force##*[![:space:]]}"}" # rtrim
    ...
    is_posi_int "$pid" || {
        error_log "Can not terminate the specified process, incorrect pid of '${pid}'" "⚠️"
        return 1
    }
...
# Description: According to its naming
current_args() {
    local array_count=$(array_count "CURRENT_ARGS")
...
current_file_path() {
    if [ -z $CURRENT_FILE_PATH ] || ! is_file "$CURRENT_FILE_PATH"; then
        printf '%s\n' "Invalid current file path: $CURRENT_FILE_PATH"
        kill -TERM -$$ &>/dev/null &
        disown %+ &>/dev/null
        exit 1
    fi
...
# Description: According to its naming
current_dir() {
    printf '%s' $(dirname $(current_file_path))
}
...
log_flush() {
    ...
    local output_buffer=$(trim "${1:-true}")
    in_array "${output_buffer}" "true" "false" || output_buffer="true"
    local log_only=$(trim "${2:-false}")
    in_array "${log_only}" "true" "false" || log_only="false"
...
error_log() {
    local error_msg="$1"
    local emoji_label=$(trim "$2")
    is_emoji "$emoji_label" || emoji_label="❗️"
    error_msg="${emoji_label} ${error_msg}"
    log_msg "$error_msg" "true" || return 1
    return 0
}
...
error_exit() {
    ...
    [ -n "$msg" ] && {
        ...
        debug_mode_on && {
            ...
            log_add "<fg_bright_yellow>(Tracing: ${current_file_path}:${current_lineno}:${current_charno}, \
...
# Function: target_dir
# Description: Installation target_dir
target_dir() {
    local target_dir="$TARGET_PATH_PREFIX"
    local os_name=$(get_os_name)

    if [[ "$os_name" == "android" ]]; then
        target_dir="$TERMUX_TARGET_PATH_PREFIX"
    fi

    target_dir=$(file_real_path "$target_dir")
    printf '%s' "$target_dir"
}

# Function: target_path
# Description: Installation target_path
target_path() {
    printf '%s' "$(target_dir)/$(current_filename)"
}
...
# normalize_string - collapse whitespace, trim leading/trailing spaces
# Usage: normalize_string "  foo   bar   " [big_spec]
...
normalize_string() {
    ...
    big_spec=$(trim "${2:-false}")
    in_array "$big_spec" "true" "false" || big_spec="false"
...
# string_join - join array elements with a separator
# Usage: string_join "array_name" "sep" [big_spec]
...
exec_command() {
    ...
    array_shift "phrase" "cmd"
    local args=$(trim "$(printf '%s ' "${phrase[@]}")")
    ...
    if [[ "$run_in_bg" == "false" ]]; then
        if [[ "$logging" == "true" ]]; then
            debug_mode_on && eval "$(printf '%s' "$phrase")" 2>&1 || eval "$(printf '%s' "$phrase")" 2>/dev/null |
                ...
                error_log "Last execution failure was from: $phrase"
                return 1
            }
        else
            debug_mode_on && eval "$(printf '%s' "$phrase")" 2>&1 || eval "$(printf '%s' "$phrase")" 2>/dev/null || {
        ...
        pid=$(safe_fork "$phrase" "true" "$logging") || {
            error_log "Last execution failure was from: $phrase"
            return 1
        }
    fi
...
file_real_path() {
    ...
    local file=$(trim "$1")
    local check_base_existence=$(trim "${2:-false}")
...
add_env_path() {
    ...
    is_dir "$new_path" || {
        error_log "The directory '${new_path}' does not exist."
        return 1
    }
...
# is_numeric - validate Go-style numeric literals (pure Bash5, no external commands)
...
# Relies on: trim "$1" exists (per your codebase).
is_numeric() {
...
# Get file info
extract_file_info() {
    local file_path=$(trim "$1")
    file_path=$(file_real_path "$file_path")
    [ -n "$file_path" ] || return 1

    local receiver_var_name=$(trim "$2")
    is_valid_var_name "$receiver_var_name" || return 1
    ...
    local rs=()
    array_set "rs" "filename" "$filename"
    array_set "rs" "file_ext" "$file_ext"
    array_set "rs" "file_path" "$file_path"
    array_set "rs" "file_dir" "$file_dir"
    array_set "rs" "file_exists" "$file_exists"
    array_set "rs" "is_dir" "$file_is_dir"
    array_set "rs" "is_file" "$file_is_file"
    array_set "rs" "file_has_ext" "$file_has_ext"

    @set "var_ref" "${rs[@]}" || return 1
...
cp_p() {
    ...
    if ! file_exists "$source_path"; then
        error_log "Source path '${source_path}' does not exist."
        return 1
    fi

    # Check the source_path's permission
    if [[ ! -e "$source_path" ]]; then
        error_log "Permission denied on file: '${source_path}'."
        return 1
    fi

    extract_file_info "$target_path" "file_info"
    target_path=$(array_get "file_info" "file_path")
    target_dir=$(array_get "file_info" "file_dir")
...
is_indexed_array() {
    local var=$(trim "$1")
    is_valid_var_name "$var" || return 1
    local decl
...
is_array() {
    is_indexed_array "$1" || is_assoc_array "$1" || return 1
    return 0
}
...
array_dump() {
    ...
    if is_indexed_array "$array_name"; then
        ...
        return 0
    elif is_assoc_array "$array_name"; then
        local kv p
...
array_copy() {
    ...
    new_arr_ref=($(array_dump "$array_name"))
    return 0
}
...
array_set() {
    ...
    # first arg: array name (trim provided by caller)
    array_name=$(trim "$1")
    shift
    ...
    while (($# > 1)); do
        ...
        *)
        ...
        tmp_var="__array_mig_${array_name//[^a-zA-Z0-9_]/}_$$_$RANDOM"
        declare -gA "$tmp_var"
...
array_get() {
    ...
    for key in "$@"; do
        ...
        # Treat EMPTY_EXPR sentinel as explicit empty if defined
...
# --------------------------------------------
# array_values <array_name> [pattern]
...
# ------------------------------------------------------------
# array_search <array_name> <search_value>
...
array_search() {
    ...
    local strict=$(trim "$3")
    in_array "$strict" "true" "false" || strict="false"
...
array_push() {
    local array_name=$(trim "$1")
    local new_value="$2"
    local value_type=$(trim "${3:-string}")
    local unique_values=$(trim "${4:-false}")
...
indexed_array_merge() {
    ...
    # Validate arguments
    if ! is_indexed_array "$src1" || ! is_indexed_array "$src2" || ! is_valid_var_name "$dest_name"; then
        return 1
    fi
...
array_shift() {
    ...
    is_valid_var_name "$val_name" && val_ref="${arr[$first_key]}" 2>/dev/null
...
array_pop() {
    ...
    [ -n "$last_key" ] || return 1
    is_valid_var_name "$val_name" && val_ref="${arr[$last_key]}" 2>/dev/null
...
check_bash_version() {
    ...
    if [[ $result == 0 ]]; then
        error_exit "Bash version (${bash_version}) is not compatible with the version requirement (${expr})."
    fi
}
...
gdate() {
    ...
    input=$(normalize_iso_datetime "$1")
    [ -n "$input" ] || {
        printf ''
        return 1
    }
    ...
    # era = floor(y0 / 400)
    # bash division truncates toward zero; for positive years this is floor
    era_val=$((y0 / 400))
...
gen_sig_file() {
    ...
    if [ -z "$sig_file" ]; then
        if [[ "$target_file" != "$(current_file_path)" ]]; then
...
#   $2 - source file path (optional, current_file_path() used if omitted)
# Output:
...
verify_sig_file() {
    ...
    [ -n "$src_file" ] || src_file="$(current_file_path)"
...
yaml_to_array() {
    ...
    while IFS=':' read -r key value; do
        ...
        array_set "rs" "$key" "$value" || return 1
    done <<<"$input" || return 1
...
extract_url_info() {
    ...
    local rs=()
    array_set "rs" "url_scheme" "$scheme"
    array_set "rs" "url_domain" "$domain"
    array_set "rs" "unencoded_req_path" "$unencoded_req_path"
    array_set "rs" "unencoded_url" "$unencoded_url"
    array_set "rs" "encoded_req_path" "$encoded_req_path"
    array_set "rs" "encoded_url" "$encoded_url"
...
reliable_curl() {
    ...
    local CURL_USE_RAW_STATUS_CODE=$(trim "${CURL_USE_RAW_STATUS_CODE:-"false"}")
    in_array "$CURL_USE_RAW_STATUS_CODE" "true" "false" || CURL_USE_RAW_STATUS_CODE="false"
...
check_commands_list() {
    ...
    invalid_commands=($(command_exists_async "${BISU_REQUIRED_EXTERNAL_COMMANDS[@]}"))
    invalid_commands_count=$(array_count "invalid_commands")
...
release_lock() {
    ...
    flock -u 200 && saferm "$lock_file" || {
        error_log "Failed to remove lock file: ${lock_file}"
        return 1
    }
...
import_script() {
    local script=$(trim "$1")
    is_file "$script" || {
        error_log "Failed to import script: $script"
        return 1
    }
...
load_required_scripts() {
    is_array "BISU_REQUIRED_SCRIPTS" || {
        error_log "Invalid BISU_REQUIRED_SCRIPTS array." "⚠️"
        return 1
    }
...
verify_sig() {
    normalize_bool "VERIFY_BISU_SIG"
    [[ "$VERIFY_BISU_SIG" == "true" ]] && {
        local sig_status=$(verify_sig_file "$(bisu_file_path).asc" "$(bisu_file_path)")
        string_starts_with "$sig_status" "ERR_" && {
            error_exit "Invalid bisu signature, signature specific error: ${sig_status}"
        }
    }

    normalize_bool "VERIFY_UTILITY_SIG"
    [[ "$VERIFY_UTILITY_SIG" == "true" ]] && {
        local sig_status=$(verify_sig_file "$(current_file_path).asc" "$(current_file_path)")
        string_starts_with "$sig_status" "ERR_" && {
            error_exit "Invalid utility signature, signature specific error: ${sig_status}"
        }
    }
}
...
register_current_command() {
    ...
    CURRENT_COMMAND="$CURRENT_FILE_PATH $current_args"
    [ -n "$CURRENT_FILE_NAME" ] && {
        USER_CONF_DIR="${HOME}/.local/config/${CURRENT_FILE_NAME}"
    }
    [ -n "$TARGET_DIR" ] && is_dir "$TARGET_DIR" || {
        TARGET_DIR=$(target_dir)
    }

    # normalization of arrays allowed to manually modify them
    array_is_available "REQUIRED_EXTERNAL_COMMANDS" &&
        indexed_array_merge "BISU_REQUIRED_EXTERNAL_COMMANDS" "REQUIRED_EXTERNAL_COMMANDS" "BISU_REQUIRED_EXTERNAL_COMMANDS" &&
        array_unique "BISU_REQUIRED_EXTERNAL_COMMANDS"
    array_is_available "REQUIRED_SCRIPTS" &&
        indexed_array_merge "BISU_REQUIRED_SCRIPTS" "REQUIRED_SCRIPTS" "BISU_REQUIRED_SCRIPTS" && array_unique "BISU_REQUIRED_SCRIPTS"
    array_is_available "ACTIONS_RO" && indexed_array_merge "BISU_ACTIONS_RO" "ACTIONS_RO" "BISU_ACTIONS_RO" &&
        array_unique "BISU_ACTIONS_RO"
    array_is_available "AUTORUN" && indexed_array_merge "BISU_AUTORUN" "AUTORUN" "BISU_AUTORUN" &&
        array_unique "BISU_AUTORUN"
    array_is_available "EXIT_WITH_COMMANDS" &&
        indexed_array_merge "BISU_EXIT_WITH_COMMANDS" "EXIT_WITH_COMMANDS" "BISU_EXIT_WITH_COMMANDS" && array_unique "BISU_EXIT_WITH_COMMANDS"
}
...
safe_fork() {
    ...
    local async=$(trim "$2")
    in_array "$async" "true" "false" || async="true"
...
continuous_exec() {
    ...
    while [ 0 ]; do
        ...
        [ -n "$op_name" ] && [ "$count" -gt 0 ] && {
            error_log "The operation [${op_name}] has been failed. Attempting retry ${count}..." "⚠️"
        }
...
execution_time() {
    is_posi_numeric "$START_TIME" || {
        START_TIME=$(time_ms)
    }
    debug_mode_on && printf '%s\n' "Elapsed: $(echo "($(time_ms) - $START_TIME)" | bc) ms"
}
...
callfunc() {
    ...
    array_shift "current_args" "action"
    action=$(normalize_string "$action")
    [[ "$action" == "callfunc" ]] || quit

    array_shift "current_args" "func"
...
current_utility_name() {
    ...
    [ -n "$utility_name" ] || utility_name=$(current_filename)
...
current_utility_info_uri() {
    ...
    if is_bisu_file; then
        ...
        doc_uri="$UTILITY_INFO_URI"
    fi
    doc_uri=$(trim "$doc_uri")

    is_valid_uri "$doc_uri" || printf ''
    printf '%s' "$doc_uri"
}
...
last_release_date() {
    ...
    if is_bisu_file; then
        last_release_date=$(trim "$BISU_LAST_RELEASE_DATE")
    else
        last_release_date=$(trim "$LAST_RELEASE_DATE")
    fi
...
in_target_dir() {
    local current_file_path="$(current_file_path)"
    local target_path="$(target_path)"
...
integrate_ops() {
    string_to_array "$(current_args)" "current_args"
    extract_args "args"
    local parg1=$(array_get "args" 1)
    local action="$parg1"
    if [ -z "$action" ]; then
        local parg1=$(array_get "current_args" 0)
        ...
        if [ -n "$parg1" ] && [ -n parg1=$(array_get "args" "$parg1") ]; then
            action="$parg1"
        fi
    fi

    in_array "$action" "install" || verify_sig
    set_action "$action"
    autorun_start &

    local option_force=$(array_get "args" "f" "force")
    local option_yes=$(array_get "args" "y" "yes")
    local option_version=$(array_get "args" "v" "version")
    local current_file_path=$(current_file_path)
    local current_filename=$(current_filename)
    local target_path=$(target_path)
    ...
    "view-log")
        ...
        ;;
    "callfunc")
        callfunc $(printf '%s ' "$(current_args)")
        ;;
    *)
...
install_script() {
    ...
    normalize_bool "verify_sig" "false"
    if [[ "$verify_sig" == "true" ]]; then
        ...
        if ! is_file "$sig_file"; then
            local sig_file_url
            if is_bisu_file; then
                sig_file_url=$(trim "$BISU_ASC_FILE_URL")
            else
                sig_file_url=$(trim "$UTILITY_ASC_FILE_URL")
            fi

            if is_valid_url "$sig_file_url"; then
                CURL_RETRIES=5
                CURL_RETRY_DELAY=3
                local status_code=$(reliable_curl "$sig_file_url" "./${sig_file}")
                [[ "$status_code" == "200" ]] || error_exit "Failed to install $current_filename"
            fi
        fi
    fi
    ...
    if [[ "$verify_sig" == "true" ]]; then
        is_file "$sig_file" || error_exit "Failed to install $current_filename"
        verify_sig
        move_file "$sig_file" "$sig_file_target_path" || error_exit "Failed to install ${current_filename}, please check for permissions."
    fi
    move_file "$current_file_path" "$target_path" || error_exit "Failed to install ${current_filename}, please check for permissions."
    log_msg "Done."
}
